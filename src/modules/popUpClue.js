    ...
    const observer = new IntersectionObserver((entries, observe) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (entry.target.classList.contains('formula-item-trans')) {
                    entry.target.classList.remove('formula-item-trans');
                    entry.target.querySelector('formula-item-descr').style.opacity = '1';
                    //entry.target.style.transform = 'rotate(0deg) translateY(-5px)';
                } else {
